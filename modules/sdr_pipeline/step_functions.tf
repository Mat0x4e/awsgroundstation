          ...
          States = {
            ...
            StartCodeBuild = {
              ...
              Parameters = {
                ...
                BuildspecOverride = "version: 0.2\nenv:\n  variables:\n    RTSTPS_HOME: /opt/rt-stps\n    CSPP_HOME: /opt/cspp-sdr\nphases:\n  pre_build:\n    commands:\n      - echo Downloading chunk from S3...\n      - aws s3 cp s3://$INPUT_BUCKET/$INPUT_KEY /tmp/input.pcap\n      - mkdir -p /tmp/output/iq /tmp/output/satdump /tmp/output/rtstps /tmp/output/cspp\n  build:\n    commands:\n      - echo Step 1 - IQ Extraction\n      - python3 /opt/scripts/iq_extract.py /tmp/input.pcap /tmp/output/iq/baseband.cs8\n      - echo Step 2 - SatDump\n      - /opt/scripts/satdump_process.sh /tmp/output/iq/baseband.cs8 /tmp/output/satdump\n      - echo Step 2b - Upload SatDump composites to S3\n      - aws s3 sync /tmp/output/satdump/ s3://$OUTPUT_BUCKET/contacts/$CONTACT_DATE/$CONTACT_ID/satdump/chunk_$CHUNK_ID/ --sse aws:kms --sse-kms-key-id $KMS_KEY_ID --exclude '*.cadu'\n      - echo SatDump composites uploaded successfully\n      - echo Step 3 - RT-STPS (non-fatal)\n      - python3 /opt/scripts/rtstps_process.py /tmp/output/satdump/*.cadu /tmp/output/rtstps || echo WARN RT-STPS failed but SatDump composites already in S3\n      - echo Step 4 - CSPP SDR (non-fatal)\n      - python3 /opt/scripts/cspp_process.py /tmp/output/rtstps /tmp/output/cspp || echo WARN CSPP failed but SatDump composites already in S3\n  post_build:\n    commands:\n      - echo Uploading SDR/GEO results if available...\n      - if [ -d /tmp/output/cspp ] && [ \"$(ls -A /tmp/output/cspp 2>/dev/null)\" ]; then aws s3 sync /tmp/output/cspp/ s3://$OUTPUT_BUCKET/contacts/$CONTACT_DATE/$CONTACT_ID/chunks/chunk_$CHUNK_ID/ --sse aws:kms --sse-kms-key-id $KMS_KEY_ID; echo SDR upload complete; else echo No CSPP output - SatDump composites already in S3; fi\n      - aws s3 cp /tmp/output/satdump/dataset.json s3://$OUTPUT_BUCKET/contacts/$CONTACT_DATE/$CONTACT_ID/chunks/chunk_$CHUNK_ID/dataset.json --sse aws:kms --sse-kms-key-id $KMS_KEY_ID 2>/dev/null || true\n"
